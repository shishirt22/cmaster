	...
	if(isset($_POST['action'])){
		if($_POST['action']=='email') {
			...
			if(trim($_POST['email']) == "")
				...
				mysql_query("UPDATE users SET email='".mysql_escape_string($_POST['email'])."' WHERE username='".$_SESSION['username']."'");
			...
			if(trim($_POST['name']) == "")
				...
				mysql_query("UPDATE prefs SET name='".mysql_escape_string($_POST['name'])."', accept=$accept, c=$c, cpp=$cpp, java=$java, python=$python");
				header("Location: index.php?changed=1");
			}
			...
			if(trim($_POST['title']) == "" or trim($_POST['problem']) == "" or !is_numeric($_POST['time']))
				...
				$query="INSERT INTO `problems` ( `name` , `text`, `input`, `output`, `time`) VALUES ('".mysql_escape_string($_POST['title'])."', '".mysql_real_escape_string($_POST['problem'])."', '".mysql_real_escape_string($_POST['input'])."', '".mysql_real_escape_string($_POST['output'])."', '".$_POST['time']."')";
				mysql_query($query);
